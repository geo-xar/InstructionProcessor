...
{
    uint16_t number = 1;
    std::tuple<OpCodeMode, OpCodeMode, OpCodeMode> expectedModes = { OpCodeMode::Position, OpCodeMode::Position, OpCodeMode::Position };
    CHECK(ExtractOpCodeModes(number) == expectedModes);
    ...
    number = 323;
    CHECK_THROWS_AS(ExtractOpCodeModes(number), std::runtime_error);
